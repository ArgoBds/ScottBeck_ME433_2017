...
float a = 0.75;
float b = 0.25;
signed short IIR_avg,MAF,FIR;
int startTime = 0;
int go = 0;
signed short IMU_values[7];
unsigned char IMU_data[14];
signed short FIR_buffer[10] = {0};
float FIR_weights[10]={0.0046,0.0052,-0.0583,0.0502,0.4982,0.4982,0.0502,-0.0583,0.0052,0.0046};
int FIR_length = sizeof(FIR_buffer)/sizeof(short);
...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_CHECK_TIMER:
            ...
            if (appData.isReadComplete || _CP0_GET_COUNT() - startTime > (48000000 / 2 / 200)) {
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            if (appData.isReadComplete) {
                ...
                if (go == 1 && i < 200) {
                    for(k = FIR_length-1; k>0; k--){
                        FIR_buffer[k]=FIR_buffer[k-1];
                    }
                        
                    I2C_read_multiple(0x20, IMU_data, 14);
                    chars_2_ints(IMU_data, IMU_values);
                    FIR_buffer[0]=IMU_values[6];
                    MAF = 0;
                    FIR = 0;
                    for(k = 0; k<FIR_length; k++){
                        MAF = MAF + FIR_buffer[k]/FIR_length;
                        FIR = FIR + FIR_weights[k]*FIR_buffer[k];
                    }
                    IIR_avg = a*IIR_avg+b*(float)IMU_values[6];
                    
                    
                    len = sprintf(dataOut, "%d\t%d\t%d\t%d\t%d\r\n", i + 1,
                            IMU_values[6], MAF,IIR_avg,FIR);
